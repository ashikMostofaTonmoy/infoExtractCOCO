    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            keypoints_original_flattened = [el[0:2]
                                            for kp in keypoints_original for el in kp]
            ...
            keypoints_transformed_unflattened = np.reshape(
                np.array(transformed['keypoints']), (-1, 2, 2)).tolist()
            ...
            for o_idx, obj in enumerate(keypoints_transformed_unflattened):
                obj_keypoints = []
                # Iterating over keypoints in each object
                for k_idx, kp in enumerate(obj):
                    # kp - coordinates of keypoint
                    # keypoints_original[o_idx][k_idx][2] - original visibility of keypoint
                    obj_keypoints.append(
                        kp + [keypoints_original[o_idx][k_idx][2]])
        ...
        target["labels"] = torch.as_tensor(
            [1 for _ in bboxes], dtype=torch.int64)  # all objects are glue tubes
        ...
        target_original["labels"] = torch.as_tensor(
            [1 for _ in bboxes_original], dtype=torch.int64)  # all objects are glue tubes
